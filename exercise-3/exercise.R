...
personal.1940 <- USPersonalExpenditure["Personal Care", "X1940"]

# How much money was spent on Food and Tobacco in 1960?
food.1960 <- USPersonalExpenditure["Food and Tobacco", "X1960"]

# What was the highest expenditure category in 1960?
# Hint: use the `max()` function to find the largest, then compare that to the column
max <- max(USPersonalExpenditure$X1960)
USPersonalExpenditure$category[USPersonalExpenditure$X1960 == max]

# Define a function `DetectHighest` that takes in a year as a parameter, and
# returns the highest spending category of that year
DetectHighest <- function(year) {
  xyear <- paste0("X", year)
  max <- max(USPersonalExpenditure[,xyear])
  return(USPersonalExpenditure$category[USPersonalExpenditure[,xyear] == max])
}
